...
export const update = (req: Request, res: Response) => withServiceContext(async (context, commit) => {
  const { params: { id: crypto_token_id }, body } = req;
  const cryptoTokenService = new CryptoTokenService(context);
  await cryptoTokenService.update(body, crypto_token_id);
  await commit();
